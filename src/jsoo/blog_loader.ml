      ...
      section
        ...
        (List.map
           (fun ((article_name : string), _content) ->
             p
               [ a
                   ~at:At.[href !%(sprintf "/blog/%s" (name_html article_name))]
  ...
  let+ result =
      ...
      Brr_io.Fetch.(
        request
          (Request.v ~init:(Request.init ())
             !%"https://api.github.com/repos/EmileTrotignon/blog/contents/" ) )
    in
    ...
    let* contents =
      ...
      |> Array.map (fun md_url ->
             Brr_io.Fetch.(
               request
                 (Request.v ~init:(Request.init ()) (Jstr.of_string md_url)) ) )
    ...
    let contents =
      List.map
        (fun jv -> Base64.decode (Jv.to_jstr jv.%{"content"}) |> Result.get_ok)
        contents
    in
